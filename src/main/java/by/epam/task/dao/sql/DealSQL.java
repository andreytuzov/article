...
public class DealSQL {
	...
	public static final int INDEX_DEAL_COST = 4;
	...
	public static final String INSERT_DEAL = "INSERT INTO deals(d_user_id, d_car_id, d_deal_state_id, d_cost, d_date_from, d_date_to, d_comment, d_cancel_reason, d_passport_number)" 
			+ " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
	public static final String DELETE_DEAL = "DELETE FROM deals WHERE d_id = ?";
	public static final String UPDATE_DEAL = "UPDATE deals "
			+ " SET d_user_id = ?, d_car_id = ?, d_deal_state_id = ?, d_cost = ?, d_date_from = ?, d_date_to = ?, d_comment = ?, d_cancel_reason = ?, d_passport_number = ?"
			+ " WHERE d_id = ?";
